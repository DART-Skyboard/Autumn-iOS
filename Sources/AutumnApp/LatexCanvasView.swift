...
struct LatexCanvasOverlay: View {
    ...
    var body: some View {
        // TF94: was a fixed `.frame(maxWidth: 620)` card with no orientation awareness —
        // unlike the other HUD studios (StudioHostView etc.), which size themselves off
        // GeometryReader. In landscape, alongside Math Solver taking the other half of the
        // screen, the fixed-width header HStack squeezed "LATEX CANVAS" into a column too
        // narrow for the text to lay out normally, wrapping it letter-by-letter down the
        // edge. GeometryReader-driven width (matching the StudioHostView pattern) plus a
        // squeeze-resistant title fixes both the resize and the wrap.
